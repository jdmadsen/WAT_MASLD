...
if("crm_environment.RData" %in% data_list){
    ...
    load("/work/WAT_MASLD/Data/cms_objects_raw.RData")
} else {
    source("/work/WAT_MASLD/R/1_Data_load_and_filtering_snRNA.R")
    source("/work/WAT_MASLD/R/2_Data_load_and_filtering_snMultiomeRNA.R")
    save(cms1, cms2, file = "/work/WAT_MASLD/Data/cms_objects_raw.RData")
    data_list <- list.files(path = c("/work/WAT_MASLD/Data/","/work/WAT_MASLD/Data/doubletremoval/"))
}

if("crm_environment.RData" %in% data_list == FALSE){
    source("/work/WAT_MASLD/3_Create_crm_environment.R")

    save(crm, file = "../data/crm_environment.RData")
    data_list <- list.files(path = c(path = c("/work/WAT_MASLD/Data/","/work/WAT_MASLD/Data/doubletremoval/"))
}
```
...
if(exists("cms.filtered", where = crm) == FALSE){
    ...
    save(crm, file = "/work/WAT_MASLD/crm_environment_filtered_002.RData")
}
...
if("total_cells_first_filtering.RData" %in% data_list == FALSE){
    ...
    save(total_cells, file = "/work/WAT_MASLD/total_cells_first_filtering.RData")
} else{
    load("/work/WAT_MASLD/total_cells_first_filtering.RData")
}
total_cells
```
